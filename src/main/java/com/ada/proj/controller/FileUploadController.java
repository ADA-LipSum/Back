...
public class FileUploadController {
    ...
    @Operation(
            ...
            description = """
                    ...
                    **허용 형식:** 이미지·영상·PDF·HWP·문서·기타 (최대 50MB)

                    **사용 흐름**
                    1. 이 API로 파일을 업로드 → `id` 반환
                    2. 공지사항 작성 시 `attachmentIds` 배열에 `id`를 담아 전달

                    **Response:** `data.id` (공지 작성 시 사용), `data.fileUrl`
                    """
    )
    public ResponseEntity<ApiResponse<PostAttachmentResponse>> uploadNoticeAttachment(
            @Parameter(description = "첨부할 파일") @RequestPart("file") MultipartFile file,
            Authentication auth) {
        ...
        return ResponseEntity.ok(ApiResponse.ok(PostAttachmentResponse.builder()
                .id(saved.getId())
